...
val pairOfAnglesParameterKind =
    object : PairParameter<Angle, Angle>(AngleParameterKind, AngleParameterKind) {
        override fun checkConfigValue(v: Any?): Boolean {
            return when (v) {
                is Tuple -> {
                    when (v.name) {
...
val optAnglePairKind =
    object : ParameterKind<Pair<Angle, Angle>?> {
...
fun <T : Configurable<T>> pairOfAnglesParameter(
    help: String = "",
    gen: (T) -> Pair<Angle, Angle>,
): ConfigParameter<T, Pair<Angle, Angle>> {
    return ConfigParameter(pairOfAnglesParameterKind, help, gen = gen)
}

fun <T : Configurable<T>> listOfOptAnglePairsParameter(
    help: String = "",
    gen: (T) -> List<Pair<Angle, Angle>?>,
): ConfigParameter<T, List<Pair<Angle, Angle>?>> {
    val mutGen = { t: T ->
        ArrayList(gen(t))
    }
