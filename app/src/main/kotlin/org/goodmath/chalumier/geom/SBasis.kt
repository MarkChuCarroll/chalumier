...
open class SBasis<T>(b: List<Linear<T>>, val math: Math<T>) {
    ...
    fun <U, V> multiplied(
        other: SBasis<U>, operator: (T, U) -> V, umath: Math<U>, vmath: Math<V>
    ): SBasis<V> {
        val zero = operator(
            math.times(basis[0].a0, math.zero), umath.times(other.basis[0].a0, umath.zero)
        )
        ...
        for (j in 0 until other.basis.size) {
            for (i in j until j + basis.size) {
                ...
                c[i + 1] = c[i + 1].plus(
                    Linear(
                        vmath.times(tri, vmath.minusOne), vmath.times(tri, vmath.minusOne), vmath
                    ), vmath.selfBridge
                )
    ...
    fun integral(): SBasis<T> {
        ...
        for (k in basis.size - 1 downTo 0) {
            aTri = math.times(
                basis[k].hat(), math.times(
                    math.times(aTri, (k + 1).toDouble() * 0.5), math.div(math.one, (2 * k + 1).toDouble())
                )
            )
            a[k] = a[k].plus(
                Linear(
                    math.times(aTri, -0.5), math.times(aTri, 0.5), math
                ), math.selfBridge
            )
        }
    ...
    fun <U, V> compose(
        ...
        for (i in len() - 1 downTo 0) {
            ...
            result = result.times(s, vuvBridge).plus(oneMinusOtherScaled, tuvBridge.prodMath.selfBridge)
    ...
    fun solve(target: SBasis<T>, k: Int, iters: Int = 20): SBasis<T> {
        ...
        for (i in 0 until iters) {
            result = result.minus(compose(result, ttt, ttt).minus(target, ttt), ttt)
                .divided(deriv.compose(result, ttt, ttt), k)
    ...
    companion object {
        ...
        fun <T> newtonoid(
            initial: SBasis<T>, fp: (SBasis<T>) -> SBasis<T>, fpp: (SBasis<T>) -> SBasis<T>, k: Int
        ): SBasis<T> {
